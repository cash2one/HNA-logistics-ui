    ...
    $.widget('mf.manifest', {
        ...
        add: function (data, $mpItem, clearInputValue, initial) {
            ...
            for (var i = 0, length = values.length; i < length; i++) {
                ...
                if(options.verification && !options.verification.test(value)){    //对值有校验规则。
                    $item = $('<li class="mf_item mf-error" role="option" aria-selected="false" />');
                }else{
        ...
        packValues: function(){    //有校验规则的数据，二次封装
            var self = this,
                options = self.options,
                $list = self.$list,
                map = {
            ...
            var flag = true;
            $list.find('input:hidden.mf_value').each(function () {
                if(options.verification && !options.verification.test($(this).val())){
                    map.errorCode = 10000;
                }
                map.data.push($(this).val());
            });
