easySpa.require([
    ...
    app.controller('customerApprovalCtrl', [
        '$scope',
        '$route',
        'customerApprovalService',
        'customerApprovalView',
        'tableService',
        function ($scope, $route, customerApprovalService, customerApprovalView, tableService) {
            ...
            if($scope.userType === 1){
                Tablehearder = [
                    ...
                    Lang.getValByKey('customerApproval', 'customer_account_name'),
                    Lang.getValByKey('customerApproval', 'customer_code'),
                    Lang.getValByKey('customerApproval', 'customer_company_name'),
                    Lang.getValByKey('customerApproval', 'customer_price_meal'),
                    // Lang.getValByKey('customerApproval', 'customer_biz_type'),
                    Lang.getValByKey('customerApproval', 'customer_level'),
                    Lang.getValByKey('customerApproval', 'customer_audit_state'),
                ]
            }else{
                Tablehearder = [
                    Lang.getValByKey('common', 'common_thead_number'),
            ...
            $scope.tableModel = {
                ...
                tableHeaderSize: ['5%', '10%', '15%', '15%', '15%', '15%','15%'],
                tableBody: [],
            ...
            $scope.isLockeds = {
                data: [{ id: -1, name: '全部' }, { id: 0, name: Lang.getValByKey('customerApproval', 'customer_unlock') }, { id: 1, name: Lang.getValByKey('customerApproval', 'customer_locked') }],
            };
            $scope.stateName = '全部';
            $scope.stateId = -1;
            $scope.combosName = Lang.getValByKey('customerApproval', 'common_select_tips');
            $scope.combosId = '';
            $scope.levals = {
                data: [
                    // {id:-1,name:'无'},
                    { id: 1, name: Lang.getValByKey('customerApproval', 'customer_one_star') },
                    { id: 2, name: Lang.getValByKey('customerApproval', 'customer_two_star') },
                    { id: 3, name: Lang.getValByKey('customerApproval', 'customer_three_star') },
                    { id: 4, name: Lang.getValByKey('customerApproval', 'customer_four_star') },
                    { id: 5, name: Lang.getValByKey('customerApproval', 'customer_five_star') },
                ],
            };


            $scope.evaluateLevalName = Lang.getValByKey('customerApproval', 'customer_one_star');
            $scope.evaluateLevalId = '1';

            $scope.auditState ={
                data: [
                    {id:-1,name:Lang.getValByKey('customerApproval', 'customer_state_all')},
                    {id:0,name:Lang.getValByKey('customerApproval', 'customer_state_writing')},
                    { id: 1, name: Lang.getValByKey('customerApproval', 'customer_state_to_be_submit') },
                    { id: 2, name: Lang.getValByKey('customerApproval', 'customer_state_audit') },
                    { id: 4, name: Lang.getValByKey('customerApproval', 'customer_state_success')},
                    { id: 3, name: Lang.getValByKey('customerApproval', 'customer_state_fail') }
                ],
            };

            $scope.auditStateName = Lang.getValByKey('customerApproval', 'customer_state_audit');

            $scope.auidtMessage = ''; // 未通过原因
            $scope.authStatus = 0; // 未通过原因

            $scope.auditStatus; // 审批状态
            $scope.auditRemark=''; //审批的原因
            ...
            $scope.seniorFilter = function () {
                if ($scope.showSenior == true) {
                    ...
                    $scope.tableModel.restData.authStatus= 2;
                    $scope.auditStateName= '待审核';
                } else {
            ...
            $scope.selectCustomerCombos = function () {
                var combosData = customerApprovalService.getCombos();
                var map = [{ id: '', name: Lang.getValByKey('customerApproval', 'common_select_tips') }];
            ...
            function inferSelected() {
                ...
                if (!oldData.length) {
                    accountView.promptBox({ isDelay: true, contentDelay: Lang.getValByKey('customerApproval', 'customer_prompt_delay_tip'), type: 'errer', manualClose: true });
                    return false;
                }
                // 组织数据
            ...
            function PostDataToServer(postMethod) {
                if ($scope.tableModel.selectNumber != 0 && $('.table-box tbody tr').length != 0) {
                    ...
                    $(document).promptBox({ isDelay: true, contentDelay: Lang.getValByKey('customerApproval', 'customer_prompt_delay_tip'), type: 'errer', manualClose: true });
                }
            }
            ...
            $scope.lockCustomer = function () {
                PostDataToServer(customerApprovalService.lockCustomer);
            };

            /* 解锁客户*/
            $scope.unLockCustomer = function () {
                PostDataToServer(customerApprovalService.unlockCustomer);
            };
            ...
            $scope.deleteCustomer = function () {
                if ($scope.tableModel.selectNumber != 0 && $('.table-box tbody tr').length != 0) {
                    ...
                    $(document).promptBox({
                        ...
                        type: 'warning',
                        content: {
                            tip: Lang.getValByKey('customerApproval', 'customer_prompt_delete_tip'),
                        },
                        operation: [
                            {
                                type: 'submit',
                                ...
                                application: 'delete',
                                operationEvent: function () {
                                    PostDataToServer(customerApprovalService.deleteCustomer);
                                },
                            },
                        ],
                    });
                        ...
                        operation: [
                            {
                                ...
                                operationEvent: function () {
                                    ...
                                    customerApprovalService.auditCustomers(params,function (res) {
                                        if (res.errorCode === 0) {
            ...
            function initUserInfo(value) {
                ...
                $scope.submitDisabled = false; //提交审核按钮设置为可用，不一定显示
                userInfoCtrl.init($scope, customerApprovalService, value);
            }
            $scope.swichEvent = function (index) {
                ...
                switch (index) {
                    case 0: {
                        ...
                        userInfoCtrl.init($scope, customerApprovalService, {
                            id: $scope.customerId,
                        });
                        break;
                    }
            ...
            /** tab切换组件 */
            $scope.tab = customerApprovalView.tab('#m-tab', $scope.swichEvent);
            ...
            function checkAudit() {
                ...
                $scope.auditRemark = $.trim($scope.auditRemark);
                if ($scope.auditStatus == 'rejections' && !$scope.auditRemark) {
            ...
            $scope.clearAuditRemark = function () {
                $scope.auditRemark = '';
            }
            ...
            $scope.submitAudit = function () {
                if (checkAudit()) {
                    ...
                    if($scope.auditStatus === 'pass'){ // 打回
                        customerApprovalService.auditSuccCustomers(param,function (res) {
                            if(res.errorCode === 0 ){
                                $(document).promptBox({ isDelay: true, contentDelay: res.msg, type: 'success', time: 3000 });
                                // $scope.goBackCustomer();
                                // $(document).promptBox('closePrompt');
                        ...
                        customerApprovalService.auditFailCustomers(param,function (res) {
                            if(res.errorCode === 0 ){
                                $(document).promptBox({ isDelay: true, contentDelay: res.msg, type: 'success', time: 3000 });
                                // $(document).promptBox('closePrompt');
                                $scope.submitDisabled = true;
