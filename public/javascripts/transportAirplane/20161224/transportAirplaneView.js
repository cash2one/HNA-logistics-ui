app.factory('transportAirplaneView', [function() {
    var transportAirplaneView = {};

    transportAirplaneView.promptBox = function(opt){
        $(document).promptBox(opt);
    };

    transportAirplaneView.propmtCostEvent = function(elm){
    ...
    transportAirplaneView.loadBarEvent = function(elm){
        elm.next().hide();
        elm.children('.icon-more-down').removeClass('icon-angle-down').addClass('icon-angle-right');
    ...
    transportAirplaneView.displayErrorBox = function(elm){
        if(elm.eq(0).next('.switch-list').is(":hidden")){
            transportAirplaneView.loadBarEvent(elm);
        }
    };

    return transportAirplaneView;
}]);