app.factory('transportShipView', [function() {
    var transportShipView = {};

    transportShipView.propmtCostEvent = function(elm){
    ...
    transportShipView.loadBarEvent = function(elm){
        elm.next().hide();
        elm.children('.icon-more-down').removeClass('icon-angle-down').addClass('icon-angle-right');
    ...
    transportShipView.promptBox = function(opt){
        $(document).promptBox(opt);
    };

    transportShipView.displayErrorBox = function(elm){
        scrollToErrorView($(".switch-list"));
        if(elm.eq(0).next('.switch-list').is(":hidden")){
            transportShipView.loadBarEvent(elm);
        }
    };

    return transportShipView;
}]);