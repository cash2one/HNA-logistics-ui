app.factory('shippingLineView', [function() {
    var shippingLineView = {};

    shippingLineView.propmtCostEvent = function(elm){
    ...
    shippingLineView.promptBox = function(opt){
        $(document).promptBox(opt);
    };

    shippingLineView.loadBarEvent = function(elm){
        elm.next().hide();
        elm.children('.icon-more-down').removeClass('icon-angle-down').addClass('icon-angle-right');
    ...
    shippingLineView.displayErrorBox = function(elm){
        if(elm.eq(0).next('.switch-list').is(":hidden")){
            shippingLineView.loadBarEvent(elm);
        }
    };
